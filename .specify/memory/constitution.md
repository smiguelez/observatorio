...
Sync Impact Report
==================
Version change: (plantilla sin completar) → 1.0.0  [RATIFICACIÓN INICIAL]
Rationale: El archivo almacenado solo contenía los placeholders del template.
  Esta es la primera ratificación formal de la constitución. El numeral 0.x del
  borrador (v0.2, v0.3) se preserva en el historial de enmiendas; la adopción
  formal fija la versión en 1.0.0 y adopta versionado semántico desde aquí.

Principios (todos nuevos en esta ratificación):
  ...
  + VI.   El modelo de autorización se reconstruye desde la fuente (MUST)
  + VII.  El esquema relacional se diseña sobre datos verificados (MUST)
...
Secciones agregadas: Core Principles, Governance.
Secciones removidas: [SECTION_2_NAME] y [SECTION_3_NAME] del template —
  omitidas deliberadamente. Los principios ya cubren restricciones de seguridad
  y flujo de trabajo; no se agregó contenido genérico de plantilla (a pedido).

Placeholders diferidos / TODOs: ninguno. Todas las fechas y valores resueltos.

Nota de sincronización: los templates dependientes (plan/spec/tasks/checklist)
  leen la constitución en tiempo de ejecución y no requieren cambios aquí.
-->

# Constitución del Observatorio de Oficinas Judiciales

Constitución del proyecto de reformulación del Observatorio de Oficinas
Judiciales. Cada principio usa MUST / SHOULD como términos normativos; ver
**Governance** para su interpretación.
...
posterior de esta constitución.
...
Esta constitución gobierna todas las specs, planes y tareas del proyecto de
reformulación del Observatorio de Oficinas Judiciales. En caso de conflicto
entre una spec y esta constitución, gana la constitución; la spec se corrige.

**Términos normativos.** Cada principio usa MUST / MUST NOT / SHOULD / SHOULD NOT
como términos normativos. `/speckit-converge` los evalúa literalmente: una
violación de un MUST es un hallazgo CRITICAL. Los principios se redactan con esa
consecuencia en mente — un MUST que no se piensa sostener es peor que un SHOULD
honesto.

**Enmiendas.** Una enmienda requiere: la versión nueva del principio, la razón
del cambio, y la fecha. Los principios no se eliminan en silencio.

**Versionado.** La versión de esta constitución sigue versionado semántico:
MAJOR para remociones o redefiniciones incompatibles de un principio o de la
gobernanza; MINOR para el agregado o la expansión material de un principio o
sección; PATCH para aclaraciones, correcciones de redacción y refinamientos no
semánticos.

**Revisión de cumplimiento.** Toda spec, plan y conjunto de tareas se revisa
contra esta constitución antes de aprobarse. Las decisiones abiertas que aún no
corresponden a un principio se registran en `docs/decisiones-pendientes.md`, no
en este archivo.

**Fuentes.** Los principios derivan de `docs/auditoria-app-actual.md` y
`docs/verificacion-datos-firestore.md` (auditoría del 2026-09) y de la decisión
...
- 2026-09 (v0.3): Principio III confirma magic link como tercer método de
  ...
  también seguridad de tokens de magic link (de un solo uso, expiración corta);
  el requisito de rate limiting/bloqueo por intentos fallidos se baja de MUST a
  SHOULD para la v1, con intención registrada de endurecerlo después.
- 2026-09-06 (v1.0.0): Ratificación formal. Los trece principios (I–XIII) y la
  gobernanza se adoptan sin cambios de fondo respecto del borrador v0.3; el
  numeral 0.x del borrador se cierra y la constitución pasa a versionado
  semántico a partir de esta versión.
